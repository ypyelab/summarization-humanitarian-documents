...
"""
Modified version: 
Taken from @jorgeramirez, who adapted the _binary_to_text function so that it outputs the format expected by NeuSum (https://github.com/magic282/NeuSum)
  article  => out_file.src.txt
  abstract => out_file.tgt.txt  
"""
...
def _binary_to_text_for_neusum(in_file, out_file):
    reader = open(in_file, 'rb')
    writer_src = open("%s.src.txt" % out_file, 'w', encoding = 'utf8')
    writer_tgt = open("%s.tgt.txt" % out_file, 'w', encoding = 'utf8')
    while True:
        ...
        tgt_txt =  "%s" % tf_example.features.feature["abstract"].bytes_list.value[0].decode('utf-8')
...
def _text_to_binary(in_file, out_file):
    inputs = open(in_file, 'r', encoding = 'utf8').readlines()
...
def main(function_name, in_file, out_file):
    assert function_name and in_file and out_file
    if function_name == 'binary_to_text':
        _binary_to_text_for_neusum(in_file, out_file)
    elif function_name == 'text_to_binary':
        _text_to_binary(in_file, out_file)


if __name__ == '__main__':
  if len(sys.argv) != 4:
    print ("USAGE: python data_convert.py '<function_name>' <in_file> <out_file>")
    sys.exit()
  function_name = sys.argv[1]
  in_file = sys.argv[2]
  out_file = sys.argv[3]
  main(function_name, in_file, out_file)