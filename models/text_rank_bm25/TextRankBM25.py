...
# Code taken from gensim in summarizer module
...
def _set_graph_edge_weights(graph, weight_threshold):
...
def summarize(text, ratio=0.2, word_count=None, weight_threshold = 1.e-3, split=False):
    ...
    if len(sentences) == 1:
        logger.warning("Summarization not performed since the document has already few sentence.")
        return text
